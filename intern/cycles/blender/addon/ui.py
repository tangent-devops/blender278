...
class CyclesRender_PT_layer_passes(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.prop(crl, "use_pass_crypto_object_index", text="Object Pass Idx", toggle=True)
        row.prop(crl, "use_pass_crypto_material_index", text="Material Pass Idx", toggle=True)
        row = layout.row(align=True)
        row.active = use_cpu(context)
