                                          ...
                                          int sample)
{
	...
	shader_eval_surface(kg, sd, &state, rbsdf, state.flag, SHADER_CONTEXT_MAIN, NULL, 0);
	...
	if(!kernel_data.integrator.branched) {
		...
		if(!is_sss_sample && (pass_filter & (BAKE_FILTER_DIRECT | BAKE_FILTER_INDIRECT))) {
			...
			kernel_path_surface_connect_light(kg, sd, &emission_sd, throughput, &state, &L_sample, light_linking, shadow_linking);
			...
			if(kernel_path_surface_bounce(kg, sd, &throughput, &state, &L_sample, &ray)) {
				...
				kernel_path_indirect(kg, &indirect_sd, &emission_sd, &ray, throughput, 1, &state, &L_sample, light_linking);
                                                       ...
                                                       const int pass_filter)
{
	...
	if(is_color) {
		if(is_direct || is_indirect) {
			...
			shader_eval_surface(kg, sd, state, 0.0f, 0, SHADER_CONTEXT_MAIN, NULL, 0);
	...
	switch(type) {
		...
		case SHADER_EVAL_NORMAL:
		{
			if ((sd.shader_flag & SD_SHADER_HAS_BUMP)) {
				shader_eval_surface(kg, &sd, &state, 0.f, 0, SHADER_CONTEXT_MAIN, NULL, 0);
			}
		...
		{
			shader_eval_surface(kg, &sd, &state, 0.f, 0, SHADER_CONTEXT_EMISSION, NULL, 0);
