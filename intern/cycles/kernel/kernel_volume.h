												   ...
												   float3 *throughput)
{
	...
	for(; t < ray->t; t += probe_step) {
		...
		float3 sigma_t;
		if(volume_shader_extinction_sample(kg, sd, state, new_P, &sigma_t)) {
			tr_min = min(tr_min, min(sigma_t.x, min(sigma_t.y, sigma_t.z)));
			tr_max = max(tr_max, max(sigma_t.x, max(sigma_t.y, sigma_t.z)));
                                                                                ...
                                                                                RNG *rng)
{
	...
	do
	{
		...
		float3 albedo = safe_divide_color(coeff.sigma_s, coeff.sigma_a + coeff.sigma_s);
		int channel = (int)(rphase*3.0f); /* TODO: better choice of channel */
		float a = kernel_volume_channel_get(albedo, channel);
		float r = kernel_volume_channel_get(coeff.sigma_a + coeff.sigma_s, channel) / sigma_samp;
		float qscat = a * r;
	...
	} while(fabsf(min(tp.x, min(tp.y, tp.z))) > 0.0f && t < ray->t);
