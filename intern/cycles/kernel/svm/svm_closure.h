...
ccl_device void svm_node_glass_setup(ShaderData *sd, MicrofacetBsdf *bsdf, int type, float eta, float roughness, bool refract)
{
	if(type == CLOSURE_BSDF_SHARP_GLASS_ID) {
		if(refract) {
			...
			bsdf->alpha_x = 0.0f;
			bsdf->ior = eta;
			ccl_fetch(sd, runtime_flag) |= bsdf_refraction_setup(bsdf);
		}
		else {
			bsdf->alpha_y = 0.0f;
			bsdf->alpha_x = 0.0f;
			bsdf->ior = 0.0f;
			ccl_fetch(sd, runtime_flag) |= bsdf_reflection_setup(bsdf);
		}
	}
		...
		if (refract) {
			ccl_fetch(sd, runtime_flag) |= bsdf_microfacet_beckmann_refraction_setup(bsdf);
		}
		else {
			ccl_fetch(sd, runtime_flag) |= bsdf_microfacet_beckmann_setup(bsdf);
		}
	}
	else {
		bsdf->alpha_x = roughness;
		...
		if (refract) {
			ccl_fetch(sd, runtime_flag) |= bsdf_microfacet_ggx_refraction_setup(bsdf);
		}
		else {
			ccl_fetch(sd, runtime_flag) |= bsdf_microfacet_ggx_setup(bsdf);
		}
	}
}
...
ccl_device void svm_node_closure_bsdf(KernelGlobals *kg, ShaderData *sd, float *stack, uint4 node, int path_flag, int *offset)
{
	...
	switch(type) {
		case CLOSURE_BSDF_PRINCIPLED_ID: {
			...
			/* calculate ior */
			float ior = (sd->runtime_flag & SD_RUNTIME_BACKFACING) ? 1.0f / eta : eta;
			...
			if(fabsf(average(base_color)) > CLOSURE_WEIGHT_CUTOFF) {
				if(subsurface < CLOSURE_WEIGHT_CUTOFF && diffuse_weight > CLOSURE_WEIGHT_CUTOFF) {
					...
					if(bssrdf) {
						...
						/* setup bsdf */
						sd->runtime_flag |= bssrdf_setup(bssrdf, (ClosureType)CLOSURE_BSSRDF_PRINCIPLED_ID);
					}
			...
			if(diffuse_weight > CLOSURE_WEIGHT_CUTOFF && sheen > CLOSURE_WEIGHT_CUTOFF) {
				...
				if(bsdf) {
					...
					/* setup bsdf */
					sd->runtime_flag |= bsdf_principled_sheen_setup(bsdf);
				}
			}
				...
				if(specular_weight > CLOSURE_WEIGHT_CUTOFF && (specular > CLOSURE_WEIGHT_CUTOFF || metallic > CLOSURE_WEIGHT_CUTOFF)) {
					...
					if(bsdf && extra) {
						...
						if(distribution == CLOSURE_BSDF_MICROFACET_GGX_GLASS_ID || roughness <= 0.075f) /* use single-scatter GGX */
							sd->runtime_flag |= bsdf_microfacet_ggx_aniso_fresnel_setup(bsdf);
						else /* use multi-scatter GGX */
							sd->runtime_flag |= bsdf_microfacet_multi_ggx_aniso_fresnel_setup(bsdf);
					}
				}
#ifdef __CAUSTICS_TRICKS__
			}
					...
					if(roughness <= 5e-2f || distribution == CLOSURE_BSDF_MICROFACET_GGX_GLASS_ID) { /* use single-scatter GGX */
...
#endif
						{
							...
							if(bsdf && extra) {
								...
								/* setup bsdf */
								sd->runtime_flag |= bsdf_microfacet_ggx_fresnel_setup(bsdf);
							}
						}
						...
						if(bsdf && extra) {
							...
							/* setup bsdf */
							sd->runtime_flag |= bsdf_microfacet_multi_ggx_glass_fresnel_setup(bsdf);
						}
					}
				}
#ifdef __CAUSTICS_TRICKS__
			...
			if(kernel_data.integrator.caustics_reflective || (path_flag & PATH_RAY_DIFFUSE) == 0) {
				...
				if(clearcoat > CLOSURE_WEIGHT_CUTOFF) {
					...
					if(bsdf && extra) {
						...
						/* setup bsdf */
						sd->runtime_flag |= bsdf_microfacet_ggx_clearcoat_setup(bsdf);
					}
				}
#ifdef __CAUSTICS_TRICKS__
			}
		...
		case CLOSURE_BSDF_DIFFUSE_ID: {
			...
			if(bsdf) {
				...
				if(roughness == 0.0f) {
					ccl_fetch(sd, runtime_flag) |= bsdf_diffuse_setup((DiffuseBsdf*)bsdf);
				}
				else {
					bsdf->roughness = roughness;
					ccl_fetch(sd, runtime_flag) |= bsdf_oren_nayar_setup(bsdf);
				}
			}
			break;
		}
		case CLOSURE_BSDF_TRANSLUCENT_ID: {
			...
			if(bsdf) {
				bsdf->N = N;
				ccl_fetch(sd, runtime_flag) |= bsdf_translucent_setup(bsdf);
			}
			break;
		}
		case CLOSURE_BSDF_TRANSPARENT_ID: {
			...
			if(bsdf) {
				bsdf->N = N;
				sd->runtime_flag |= bsdf_transparent_setup(bsdf);
			}
			break;
		}
			...
			if(bsdf) {
				...
				if (type == CLOSURE_BSDF_REFLECTION_ID) {
					...
					if(bsdf->extra) {
						...
						ccl_fetch(sd, runtime_flag) |= bsdf_microfacet_multi_ggx_setup(bsdf);
					}
				}
				else
					ccl_fetch(sd, runtime_flag) |= bsdf_ashikhmin_shirley_setup(bsdf);
			}
		...
		case CLOSURE_BSDF_MICROFACET_MULTI_GGX_GLASS_ID: {
			...
			if(bsdf && extra) {
				...
				bsdf->alpha_y = param1;
				float eta = fmaxf(param2, 1e-5f);
				bsdf->ior = (ccl_fetch(sd, runtime_flag) & SD_RUNTIME_BACKFACING)? 1.0f/eta: eta;

				kernel_assert(stack_valid(data_node.z));
				bsdf->extra->color = stack_load_float3(stack, data_node.z);

				/* setup bsdf */
				ccl_fetch(sd, runtime_flag) |= bsdf_microfacet_multi_ggx_glass_setup(bsdf);
			}
		...
		case CLOSURE_BSDF_ASHIKHMIN_SHIRLEY_ANISO_ID: {
			...
			if(bsdf) {
				...
				if(type == CLOSURE_BSDF_MICROFACET_BECKMANN_ANISO_ID) {
					ccl_fetch(sd, runtime_flag) |= bsdf_microfacet_beckmann_aniso_setup(bsdf);
				}
				else if(type == CLOSURE_BSDF_MICROFACET_GGX_ANISO_ID) {
					ccl_fetch(sd, runtime_flag) |= bsdf_microfacet_ggx_aniso_setup(bsdf);
				}
					...
					if(bsdf->extra) {
						...
						ccl_fetch(sd, runtime_flag) |= bsdf_microfacet_multi_ggx_aniso_setup(bsdf);
					}
				}
				else {
					ccl_fetch(sd, runtime_flag) |= bsdf_ashikhmin_shirley_aniso_setup(bsdf);
				}
			}
			break;
		}
		case CLOSURE_BSDF_ASHIKHMIN_VELVET_ID: {
			...
			if(bsdf) {
				...
				ccl_fetch(sd, runtime_flag) |= bsdf_ashikhmin_velvet_setup(bsdf);
			}
			break;
		}
		...
		case CLOSURE_BSDF_DIFFUSE_TOON_ID: {
			...
			if(bsdf) {
				...
				if (type == CLOSURE_BSDF_DIFFUSE_TOON_ID) {
					ccl_fetch(sd, runtime_flag) |= bsdf_diffuse_toon_setup(bsdf);
				}
				else {
					ccl_fetch(sd, runtime_flag) |= bsdf_glossy_toon_setup(bsdf);
				}
			}
			break;
		}
		...
		case CLOSURE_BSDF_HAIR_TRANSMISSION_ID: {
			...
			if ((ccl_fetch(sd, runtime_flag) & SD_RUNTIME_BACKFACING) && (ccl_fetch(sd, type) & PRIMITIVE_ALL_CURVE)) {
				...
				if(bsdf) {
					...
					ccl_fetch(sd, runtime_flag) |= bsdf_transparent_setup(bsdf);
				}
			}
			else {
				...
				if(bsdf) {
					...
					if(type == CLOSURE_BSDF_HAIR_REFLECTION_ID) {
						ccl_fetch(sd, runtime_flag) |= bsdf_hair_reflection_setup(bsdf);
					}
					else {
						ccl_fetch(sd, runtime_flag) |= bsdf_hair_transmission_setup(bsdf);
					}
				}
			}
...
ccl_device void svm_node_closure_volume(KernelGlobals *kg, ShaderData *sd, float *stack, uint4 node, int path_flag)
{
	...
	switch(type) {
		case CLOSURE_VOLUME_ABSORPTION_ID: {
			...
			if(sc) {
				ccl_fetch(sd, runtime_flag) |= volume_absorption_setup(sc);
			}
			break;
		}
		case CLOSURE_VOLUME_HENYEY_GREENSTEIN_ID: {
			...
			if(volume) {
				volume->g = param2; /* g */
				ccl_fetch(sd, runtime_flag) |= volume_henyey_greenstein_setup(volume);
			}
			break;
		}
		default:
...
ccl_device void svm_node_closure_emission(ShaderData *sd, float *stack, uint4 node)
{
	...
	ccl_fetch(sd, runtime_flag) |= SD_RUNTIME_EMISSION;
}
...
ccl_device void svm_node_closure_holdout(ShaderData *sd, float *stack, uint4 node)
{
	...
	ccl_fetch(sd, runtime_flag) |= SD_RUNTIME_HOLDOUT;
}
