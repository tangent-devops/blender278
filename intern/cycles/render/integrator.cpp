...
void Integrator::device_update(Device *device, DeviceScene *dscene, Scene *scene)
{
	...
	if(transparent_shadows) {
		...
		foreach(Shader *shader, scene->shaders) {
			...
			if ((shader->has_surface_transparent && shader->use_transparent_shadow) || shader->has_volume || shader->use_uniform_alpha) {
	...
	kintegrator->use_ambient_occlusion =
		(scene->film->PS.contains(PASS_AO) || dscene->data.background.ao_factor != 0.0f);
